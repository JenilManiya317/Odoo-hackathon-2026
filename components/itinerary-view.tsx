...
import { ArrowDownUp, CircleUserRound, Copy, Filter, Grip, Loader2, MapPin, Search, Wallet } from 'lucide-react'
...
export default function ItineraryView() {
  ...
  useEffect(() => {
    async function loadItinerary() {
      ...
      const [{ data: trip }, { data: stops, error }] = await Promise.all([
        supabase.from('trips').select('name, is_public').eq('id', tripId).maybeSingle(),
      ...
      if (trip) setIsPublic(Boolean(trip.is_public))
  ...
  async function toggleSharing() {
    ...
    const { error } = await createClient().from('trips').update({ is_public: nextValue }).eq('id', tripId)
    if (!error) setIsPublic(nextValue)
  }
    ...
    <main className="min-h-screen overflow-x-hidden bg-background px-3 py-3 text-foreground sm:px-6 sm:py-6">
      <div className="mx-auto max-w-[980px] overflow-hidden rounded-[26px] border border-border bg-card shadow-2xl shadow-black/20">
        ...
        <section className="px-4 py-6 sm:px-9 sm:py-8">
          <div className="text-center"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-accent">{tripName}</p><h1 className="mt-2 font-serif text-3xl sm:text-4xl">Your itinerary</h1><div className="mt-3 flex flex-wrap justify-center gap-4"><Link href={tripId ? `/trips/itinerary?tripId=${tripId}&edit=1` : '/trips'} className="text-sm font-semibold text-accent hover:underline">Build or edit itinerary</Link>{tripId && <button onClick={toggleSharing} className="inline-flex items-center gap-1.5 text-sm font-semibold text-accent hover:underline"><Copy size={14} />{isPublic ? 'Unpublish itinerary' : 'Share itinerary'}</button>}{isPublic && tripId && <Link href={`/shared/${tripId}`} className="text-sm font-semibold text-accent hover:underline">Open public view</Link>}</div></div>
